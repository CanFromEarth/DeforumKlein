...
def get_inbetweens(key_frames, max_frames, integer=False, interp_method='Linear', is_single_string = False):
    import numexpr
    if is_single_string:
        key_frame_series = pd.Series([np.nan for a in range(max_frames)], dtype=object)
    else:
        key_frame_series = pd.Series([np.nan for a in range(max_frames)], dtype=float)
    ...
    for i in range(0, max_frames):
        ...
        if not value_is_number:
            ...
            if is_single_string:
                ...
                if value.find('"') > -1:
                    value = value.replace('"',"")
            key_frame_series[i] = numexpr.evaluate(value) if not is_single_string else value
