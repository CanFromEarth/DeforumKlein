#!/usr/bin/env python3
"""
run_klein.py - Deforum animation with FLUX.2-klein-4B

Entry point for running Deforum animations using the FLUX.2-klein-4B model
via HuggingFace diffusers. Drop-in replacement for run.py.

Usage:
    # Option A: just run (auto-installs deps)
    python run_klein.py

    # Option B: if model is gated, set HF token first
...
def _ensure_diffusers():
    """Ensure diffusers is installed with FLUX.2 support."""
    try:
        from diffusers import Flux2KleinPipeline  # noqa: F401
        return
    except ImportError:
        pass
    # Need latest diffusers from git for FLUX.2 support
...
# Also make sure ffmpeg is available for video export
...
try:
    ...
    # Running headless — replace IPython.display with safe no-ops
    import types
    ...
    # Also patch the top-level 'IPython' module so `from IPython import display` works
...
# Set up module paths — no x-flux submodule needed for FLUX.2-klein
sys.path.extend(['./deforum_flux', './deforum_flux/src'])

# === Redirect generate imports to klein version BEFORE importing render ===
# This makes render.py use generate_klein.generate() transparently.
import helpers.generate_klein
sys.modules['helpers.generate'] = sys.modules['helpers.generate_klein']
...
def PathSetup():
    output_path = "outputs"
    models_path = "./models"  # for MiDaS depth maps etc.
    return locals()
...
def ModelSetup():
    ...
    # Set offload=True for GPUs with <=24GB, False for 40GB+ (A100 etc.)
    gpu_mem = 0
    if torch.cuda.is_available():
        gpu_mem = torch.cuda.get_device_properties(0).total_mem / 1e9
    use_offload = gpu_mem < 30

    klein = KleinModel(offload=use_offload)
    pipe = klein.pipe
    return locals()
...
# --------------------------------------------------------------------------- #
def DeforumAnimArgs():
    animation_mode = '2D'  # '2D', '3D', 'Video Input', 'Interpolation', 'None'
    max_frames = 120
    border = 'replicate'
    ...
    # Strength schedule — tuned for 8 steps (granularity ≈ 0.125 per step)
    # Range [0.5 .. 0.875] keeps frames coherent while allowing change
    ...
    sampler_schedule = "0:('Klein Default Scheduler')"
    ...
    # Interpolation (limited support — Qwen3 embedding interpolation not yet implemented)
...
# FLUX.2-klein is distilled: no classifier-free guidance, negative prompts are ignored.
neg_prompts = {
    0: "",
}
...
# %%
# --------------------------------------------------------------------------- #
#  Generation settings — tuned for FLUX.2-klein-4B
# --------------------------------------------------------------------------- #
def DeforumArgs():
    W = 1024
    H = 1024
    ...
    save_samples = True
    save_settings = True
    display_samples = False  # headless on RunPod
